...
void *tree_sitter_jai_external_scanner_create() { return NULL; }

unsigned tree_sitter_jai_external_scanner_serialize(void *payload, char *buffer) { return 0; }
...
bool tree_sitter_jai_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    if (valid_symbols[FLOAT]) {
        ...
        // basically, -? [0-9]+ \. [0-9]*, BUT a second . after isn't allowed
        // cuz it could be ..< operator,
        // and exponents, [eE][+-]?[0-9]+
        ...
        bool found_number_after_exponent = false;
        for (int i = 0;; i++) {
            switch (lexer->lookahead) {
                ...
                case '+':
                case '-':
                    if (i == 0 || (found_exponent && !found_number_after_exponent)) {
                        advance(lexer);
                    } else {
                        goto newline;
                    }
